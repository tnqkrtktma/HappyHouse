...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#mvWriteBtn').focusin(function() {
			$(location).attr("href", "${root}/board/noticewrite");
		});
		...
		$("#searchBtn").click(function() {
			...
			$("#searchform").attr("action", "${root}/board/noticelist").submit();
		});

		$(".page-item").click(function() {
			$("#pg").val(($(this).attr("data-pg")));
			$("#pageform").attr("action", "${root}/board/noticelist").submit();
		});
				...
				<tbody>
					<c:if test="${articles.size() != 0}">
						<c:forEach var="article" items="${articles}">
							<tr onClick="location.href='${root}/board/noticedetail?articleno=${article.articleno}' ">
