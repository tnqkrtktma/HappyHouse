	...
	<header class="masthead" style="height: 30">
		<div class="container h-30">
			<div class="row h-30 align-items-center justify-content-center text-center">
				<div class="col-lg-10">
					<h1 class="text-uppercase text-white font-weight-bold">아파트 매매 정보</h1>
			...
			<div class="card col-sm-12 mt-1" style="min-height: 850px;">
				...
				$(document).ready(function(){
					...
					$("#dong").change(function() {
						$.get("${pageContext.request.contextPath}/map/aptTrade"
								,{action:"search", dong:$("#dong").val(), type:1}
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append(
										"<tr class='text-center'><th>아파트명</th><th>매매가(만원)</th><th>면적(㎡)</th><th>거래일자</th><th>상세보기</th></tr>"
										);
									console.log(data);
									$.each(data, function(index, vo) {
					...
					$("#find").click(function() {
						$.get("${pageContext.request.contextPath}/map/aptTradefind"
								,{sido:$("#sido").val(),gugun:$("#gugun").val(), dong:$("#dong").val(),name:$("#name").val(),type:1}
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append(
										"<tr class='text-center'><th>아파트명</th><th>매매가(만원)</th><th>면적(㎡)</th><th>거래일자</th><th>상세보기</th></tr>"
				...
				function detail(no)
				{
					location.href="http://localhost:8000/happyhouse/article/detail?sido="+$('#sido').val()+"&gugun="+$('#gugun').val()
						+"&dong="+$('#dong').val()+"&name="+$('#name').val()+"&no="+no+"&type=1";
				}
