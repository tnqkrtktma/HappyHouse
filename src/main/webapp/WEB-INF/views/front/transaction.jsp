	...
	<header class="masthead" style="height: 30">
		<div class="container h-30">
			<div class="row h-30 align-items-center justify-content-center text-center">
				<div class="col-lg-10">
					<h1 class="text-uppercase text-white font-weight-bold">실거래가 정보</h1>
			...
			<div class="card col-sm-12 mt-1" style="min-height: 850px;">
				...
				$(document).ready(function(){
					...
					$("#dong").change(function() {
						$.get("${pageContext.request.contextPath}/map/apt"
								...
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append("<tr class='text-center'><th>동</th><th>아파트명</th><th>지번</th><th>상세보기</th></tr>");
									$.each(data, function(index, vo) {
										let str = "<tr class="+colorArr[index%3]+">"
										+ "<td  class='text-center'>" + vo.dong + "</td>"
					...
					$("#find").click(function() {
						$.get("${pageContext.request.contextPath}/map/find"
								,{sido:$("#sido").val(),gugun:$("#gugun").val(), dong:$("#dong").val(),name:$("#name").val()}
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append("<tr class='text-center' ><th>동</th><th>아파트명</th><th>지번</th><th>상세보기</th></tr>");
									$.each(data, function(index, vo) {
				...
				function detail(no)
				{
					location.href="http://localhost:8000/happyhouse/article/detail?sido="+$('#sido').val()+"&gugun="+$('#gugun').val()
						+"&dong="+$('#dong').val()+"&name="+$('#name').val()+"&no="+no+"&type=0";
				}
					...
					<div class="col-lg-7 order-lg-2 ml-xl-auto">
						...
						<script>
							...
							function addMarker(tmpLat, tmpLng, aptName) {
								...
								now= {lat: parseFloat(tmpLat), lng: parseFloat(tmpLng)};
