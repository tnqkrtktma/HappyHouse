	...
	<header class="masthead" style="height: 30">
		<div class="container h-30">
			<div class="row h-30 align-items-center justify-content-center text-center">
				<div class="col-lg-10">
					<h1 class="text-uppercase text-white font-weight-bold">아파트 전월세 정보</h1>
...
// 						
						$.get("${pageContext.request.contextPath}/map/aptRent"
								,{action:"search", dong:$("#dong").val(), type:2}
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append(
										"<tr class='text-center'><th>아파트명</th><th>보증금/월세(만원)</th><th>면적(㎡)</th><th>거래일자</th><th>상세보기</th></tr>"
										);
									$.each(data, function(index, vo) {
					...
					$("#find").click(function() {
						$.get("${pageContext.request.contextPath}/map/aptRentfind"
								...
								,function(data, status){
									...
									$.each(data, function(index, vo) {
										if(vo.rentMoney=="null")
										{
											let str = "<tr class="+colorArr[index%3]+">"
											+ "<td class='text-center'>" + vo.aptName + "</td>" // 아파트이름
											+ "<td class='text-center'>" + vo.dealAmount +"/" + vo.rentMoney + "(만원)</td>"	// 금액
											+ "<td class='text-center'>" + vo.area + "</td>"	// 면적
											+ "<td class='text-center'>" + vo.dealYear +"."+vo.dealMonth + "." + vo.dealDay + "</td>"	// 거래일자
											+ "<td  class='text-center'><button onclick='detail("+vo.no+")'>상세보기</button></td>"
											+ "</tr>";
				...
				function detail(no)
				{
					location.href="http://localhost:8000/happyhouse/article/detail?sido="+$('#sido').val()+"&gugun="+$('#gugun').val()
						+"&dong="+$('#dong').val()+"&name="+$('#name').val()+"&no="+no+"&type=2";
				}
				...
				<div class="row" align="center">
					<aside class="col-lg-5 order-lg-1">
						<h4><strong>아파트 전월세 정보</strong></h4>
