	...
	<header class="masthead" style="height: 30">
		<div class="container h-30">
			<div class="row h-30 align-items-center justify-content-center text-center">
				<div class="col-lg-10">
					<h1 class="text-uppercase text-white font-weight-bold">주택 전월세 정보</h1>
			...
			<div class="card col-sm-12 mt-1" style="min-height: 850px;">
				...
				$(document).ready(function(){
					...
					$("#dong").change(function() {
						$.get("${pageContext.request.contextPath}/map/houseRent"
								,{action:"search", dong:$("#dong").val(), type:4}
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append(
// 											"<caption style='caption-side: top; text-align: center; font-size:1.4em;'><strong>" + AptName + "</strong></caption>" +
										"<tr class='text-center'><th>주택명</th><th>보증금/월세(만원)</th><th>면적(㎡)</th><th>거래일자</th><th>상세보기</th></tr>"
										);
									console.log(data);
									$.each(data, function(index, vo) {
										if(vo.rentMoney=="null")
										{
											let str = "<tr class="+colorArr[index%3]+">"
											+ "<td class='text-center'>" + vo.aptName + "</td>" // 아파트이름
											+ "<td class='text-center'>" + vo.dealAmount + "(전세)</td>"	// 금액
										...
										else
										{
											let str = "<tr class="+colorArr[index%3]+">"
											+ "<td class='text-center'>" + vo.aptName + "</td>" // 아파트이름
											+ "<td class='text-center'>" + vo.dealAmount +"/"+ vo.rentMoney + "</td>"	// 금액
											+ "<td class='text-center'>" + vo.area + "</td>"	// 면적
											+ "<td class='text-center'>" + vo.dealYear +"."+vo.dealMonth + "." + vo.dealDay + "</td>"	// 거래일자
											+ "<td  class='text-center'><button onclick='detail("+vo.no+")'>상세보기</button></td>"
											+ "</tr>";
					...
					$("#find").click(function() {
						$.get("${pageContext.request.contextPath}/map/houseRentfind"
								,{sido:$("#sido").val(),gugun:$("#gugun").val(), dong:$("#dong").val(),name:$("#name").val(),type:4}
								,function(data, status){
									$("#searchResult").empty();
									$("#searchResult").append(
// 											"<caption style='caption-side: top; text-align: center; font-size:1.4em;'><strong>" + AptName + "</strong></caption>" +
				...
				function detail(no)
				{
					location.href="http://localhost:8000/happyhouse/article/detail?sido="+$('#sido').val()+"&gugun="+$('#gugun').val()
						+"&dong="+$('#dong').val()+"&name="+$('#name').val()+"&no="+no+"&type=4";
				}
				...
				<div class="row" align="center">
					<aside class="col-lg-5 order-lg-1">
						<h4><strong>주택 전월세 정보</strong></h4>
						<table class="table mt-3 text-center" id="searchResult">
...
// 								console.log(lat, lng, aptName);
								$.get("${pageContext.request.contextPath}/map"
										,{act:"aptdeals", dong:$("#dong").val(), aptName:AptName}
										,function(data, status){
											$("#searchResult").empty();
											$("#searchResult").append("<caption style='caption-side: top; text-align: center; font-size:1.4em;'><strong>" + AptName + "</strong></caption>"
												+ "<tr class='text-center'><th>금액</th><th>면적</th><th>거래일자</th></tr>"
												);
											$.each(data, function(index, vo) {
												let str = "<tr class="+colorArr[index%3]+">"
// 												+ "<td>" + vo.aptName + "</td>" // 아파트이름
												+ "<td>" + vo.dealAmount + "만원</td>"	// 금액
												+ "<td>" + vo.area + "</td>"	// 면적
												+ "<td>" + vo.dealYear +"."+vo.dealMonth + "." + vo.dealDay + "</td>"	// 거래일자
												+ "</tr>";
												$("#searchResult").append(str);
											});//each
											$("#searchResult").append("</tbody>");
